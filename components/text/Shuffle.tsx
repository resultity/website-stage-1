...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  const wrappersRef: React.MutableRefObject<HTMLElement[]> = useRef<HTMLElement[]>([]);
  ...
  useEffect(() => {
    ...
    if ("fonts" in docWithFonts && docWithFonts.fonts) {
      if (docWithFonts.fonts.status === "loaded") setFontsLoaded(true);
      else docWithFonts.fonts.ready.then(() => setFontsLoaded(true));
    } else {
      setFontsLoaded(true);
    }
  }, []);
  ...
  return React.createElement(
    TagEl,
    { ref: refCallback, className: classes, style: commonStyle },
    text
  );
};
