...
type GradientColor = keyof typeof gradients;
...
export default function GradientBox({
  ...
  const bgList = fillColor ? gradients[fillColor] : [];
  const borderList = borderColor ? gradients[borderColor] : [];

  const fill = fillColor
    ? bgList[Math.max(0, Math.min(5, (fillVariant ?? 1) - 1))]
    : undefined;

  const borderGradient = borderColor
    ? borderList[Math.max(0, Math.min(5, (borderVariant ?? 1) - 1))]
    : undefined;
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          padding,
          borderRadius: radius,
          backgroundColor: fill ? 'rgba(255,255,255,0.05)' : 'transparent',
          backdropFilter: fill ? 'blur(8px)' : undefined,
          WebkitBackdropFilter: fill ? 'blur(8px)' : undefined,
        }}
      >
        {children}
      </div>
