// components/unisection/PixelBlast.tsx
// Notes:
// - Defensive guards ensure animation stops cleanly on unmount/reinit
// - All listeners are tied to an AbortController for proper cleanup
// - Render loop checks for disposed/NULL refs and WebGL context loss
// - Event handlers no-op after disposal to avoid racing frames
// - TypeScript strict: no explicit `any`; safe type guards added
...
const createTouchTexture = () => {
  ...
  type TrailPoint = { x: number; y: number; vx: number; vy: number; force: number; age: number }
...
float vnoise(vec3 p){
  ...
  float x00=mix(n000,n100,w.x), x10=mix(n010,n110,w.x), x01=mix(n001,n101,w.x), x11=mix(n011,n111,w.x);
...
const MAX_CLICKS = 10
const isEffectPass = (p: Pass): p is EffectPass => p instanceof EffectPass

const hasIsContextLost = (
  ctx: WebGL2RenderingContext | null
): ctx is WebGL2RenderingContext & { isContextLost: () => boolean } =>
  !!ctx && typeof (ctx as WebGL2RenderingContext & { isContextLost?: () => boolean }).isContextLost === 'function'
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        ...
        t.composer?.dispose()
        t.renderer.dispose()
        if (t.renderer.domElement.parentElement === parentAtMount) parentAtMount.removeChild(t.renderer.domElement)
      ...
      parentAtMount.appendChild(renderer.domElement)

      const uniforms: UniformsBag = {
      ...
      const setSize = () => {
        if (!parentAtMount || !renderer) return
        const w = parentAtMount.clientWidth || 1
        const h = parentAtMount.clientHeight || 1
      ...
      if (liquid) {
        ...
        effectsToTick.push(liquidEffect)
      ...
      if (noiseAmount > 0) {
        ...
        const noiseEffect = new Effect(
          ...
          { uniforms: new Map<string, THREE.Uniform>([['uTime', new THREE.Uniform(0)], ['uAmount', new THREE.Uniform(noiseAmount)]]) }
        )
        effectsToTick.push(noiseEffect)
        const noisePass = new EffectPass(camera, noiseEffect)
        noisePass.renderToScreen = true
        if (composer && composer.passes.length > 0) {
          composer.passes.forEach((p: Pass) => { if (isEffectPass(p)) p.renderToScreen = false })
        }
      ...
      const mapToPixels = (e: PointerEvent) => {
        ...
        const fyFlipped = renderer.domElement.height - fy
        return { fx, fy: fyFlipped, w: renderer.domElement.width, h: renderer.domElement.height }
      }
      ...
      const onPointerMove = (e: PointerEvent) => {
        const t = threeRef.current
        if (!t || t.disposed || !touch) return
        const { fx, fy, w, h } = mapToPixels(e)
        touch.addTouch({ x: fx / w, y: fy / h })
      }

      const onVisibility = () => { visibilityRef.current.visible = document.visibilityState !== 'hidden' }
      const onContextLost = (ev: Event) => { const t = threeRef.current; if (!t) return; ev.preventDefault(); t.disposed = true }

      renderer.domElement.addEventListener('pointerdown', onPointerDown, { passive: true, signal: abortCtl.signal })
      renderer.domElement.addEventListener('pointermove', onPointerMove, { passive: true, signal: abortCtl.signal })
      document.addEventListener('visibilitychange', onVisibility, { passive: true, signal: abortCtl.signal })
      renderer.domElement.addEventListener('webglcontextlost', onContextLost, { passive: false, signal: abortCtl.signal })
      window.addEventListener('pagehide', () => { const t = threeRef.current; if (!t) return; t.disposed = true }, { passive: true, signal: abortCtl.signal })
      ...
      const animate = () => {
        ...
        const ctx = (t.renderer.getContext() as WebGL2RenderingContext | null) || null
        if (!ctx || (hasIsContextLost(ctx) && ctx.isContextLost())) { t.disposed = true; return }
        if (!t.renderer.domElement.isConnected) { t.disposed = true; return }
        ...
        if (t.liquidEffect) {
          ...
          if (uTime) (uTime as THREE.Uniform<number>).value = t.uniforms.uTime.value
        }

        for (const eff of t.effectsToTick) {
          const u = eff.uniforms.get('uTime')
          if (u) (u as THREE.Uniform<number>).value = t.uniforms.uTime.value
        }
      ...
      if (t.liquidEffect) {
        ...
        if (uStrength) (uStrength as THREE.Uniform<number>).value = liquidStrength
        const uFreq = t.liquidEffect.uniforms.get('uFreq')
        if (uFreq) (uFreq as THREE.Uniform<number>).value = liquidWobbleSpeed
      }
    ...
    return () => {
      ...
      if (t.renderer.domElement.parentElement === parentAtMount) {
        try { parentAtMount.removeChild(t.renderer.domElement) } catch {}
      }
      threeRef.current = null
    }
  }, [
