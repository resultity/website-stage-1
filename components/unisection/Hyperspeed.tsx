// ./components/unisection/Hyperspeed.tsx
...
/* Shared uniform typing to avoid `any` */
type UniformValue =
  | number
  | boolean
  | THREE.Color
  ...
  | THREE.Texture
  | ArrayLike<number>;
type UniformDict = Record<string, { value: UniformValue }>;
...
/* Distortion uniforms with strict types */
const mountainUniforms: UniformDict = {
  uFreq: { value: new THREE.Vector3(3, 6, 10) },
  uAmp: { value: new THREE.Vector3(30, 30, 20) },
};
const xyUniforms: UniformDict = {
  uFreq: { value: new THREE.Vector2(5, 2) },
  uAmp: { value: new THREE.Vector2(25, 15) },
};
const LongRaceUniforms: UniformDict = {
  uFreq: { value: new THREE.Vector2(2, 3) },
  uAmp: { value: new THREE.Vector2(35, 10) },
};
const turbulentUniforms: UniformDict = {
  uFreq: { value: new THREE.Vector4(4, 8, 8, 1) },
  uAmp: { value: new THREE.Vector4(25, 5, 10, 10) },
};
const deepUniforms: UniformDict = {
...
const distortion_uniforms: UniformDict = {
...
/* NEW: safely build InstancedBufferGeometry from BufferGeometry */
function makeInstancedFrom(geom: THREE.BufferGeometry): THREE.InstancedBufferGeometry {
  const inst = new THREE.InstancedBufferGeometry();

  const index = geom.getIndex();
  if (index) inst.setIndex(index);

  for (const name of Object.keys(geom.attributes)) {
    const attr = geom.getAttribute(name);
    if (attr) inst.setAttribute(name, attr);
  }
  inst.groups = geom.groups.map(g => ({ start: g.start, count: g.count, materialIndex: g.materialIndex }));
  ...
  const dr = (geom as any).drawRange as { start: number; count: number } | undefined;
  if (dr) inst.setDrawRange(dr.start, dr.count);
...
class CarLights {
  ...
  init() {
    ...
    // FIX: no .copy(BufferGeometry)
    const instanced = makeInstancedFrom(geometry as unknown as THREE.BufferGeometry);
  ...
  shader: Parameters<THREE.ShaderMaterial['onBeforeCompile']>[0]
) => {
...
class LightsSticks {
  ...
  init() {
    ...
    const sticksColorArr: THREE.Color[] = Array.isArray(options.colors.sticks)
    ...
    for (let i = 0; i < totalSticks; i++) {
      ...
      const color = pickRandom<THREE.Color>(sticksColorArr);
      aColor.push(color.r, color.g, color.b);
...
class Road {
  ...
  createPlane(side: number, _width: number, isRoad: boolean) {
    ...
    const roadAlpha =
      isRoad &&
      (options.colors.roadColor === 0x00000000 || options.colors.roadColor === 0x888888)
        ? 0.0
        : 1.0;
    
let uniforms: UniformDict = {
  uTravelLength: { value: options.length },
  uColor: {
  ...
  uTime: { value: this.uTime.value }, // fixed
  uAlpha: { value: isRoad ? roadAlpha : 1.0 },
};
    uniforms.uTime = { value: this.uTime.value };
...
class App {
  ...
  fogUniforms: UniformDict;
  fovTarget: number;
  speedUpTarget: number;
  speedUp: number;
  timeOffset: number;
  // keep the same reference to remove properly
  ...
  setSize(width: number, height: number, updateStyles: boolean) {
    // Resize both renderer (needs updateStyles) and composer (2 args)
...
/* Component with memoized options to stabilize useEffect deps */
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  const hyperspeed = useRef<HTMLDivElement>(null);
  const appRef = useRef<App | null>(null);

  const mergedOptions = useMemo<HyperspeedOptions>(() => {
    return { ...defaultOptions, ...effectOptions };
  }, [effectOptions]);
